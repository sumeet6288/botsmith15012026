...
import { DollarSign, TrendingUp, CreditCard, Users, AlertCircle, RefreshCw } from 'lucide-react';
...
const RevenueDashboard = ({ backendUrl }) => {
  ...
  const fetchRevenueData = async () => {
    try {
      ...
      console.log('Fetching revenue data from:', backendUrl);
      
      // Fetch overview
      const overviewRes = await fetch(`${backendUrl}/api/admin/revenue/overview`, { headers });
      if (!overviewRes.ok) {
      ...
      // Fetch history
      const historyRes = await fetch(`${backendUrl}/api/admin/revenue/history?days=30`, { headers });
      if (!historyRes.ok) {
      ...
      // Set default empty data to prevent UI crashes
      setOverview({
        mrr: 0,
        arr: 0,
        total_revenue: 0,
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12 space-y-4">
        ...
        <div className="text-center">
          <h3 className="text-lg font-semibold text-gray-900 mb-2">Failed to Load Revenue Data</h3>

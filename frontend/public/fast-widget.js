(function() {
  ...
  style.textContent = `
    ...
    .botsmith-streaming-cursor {
      ...
      vertical-align: -3px;
      border-radius: 2px;
      animation: botsmithCursorBlink 0.8s step-end infinite;
    }
  ...
  async function streamAssistantMessage(content) {
    ...
    msgDiv.innerHTML = `
      ...
      <div style="max-width: 72%; padding: 14px 18px; border-radius: ${bubbleRadius}; background: white; box-shadow: 0 3px 12px rgba(0, 0, 0, 0.08), 0 1px 3px rgba(0, 0, 0, 0.05); word-wrap: break-word; font-size: ${fontSize}; line-height: 1.6; font-family: ${customization.font_family}; border: 1px solid rgba(0, 0, 0, 0.05);">
        <span class="botsmith-streaming-content"></span><span class="botsmith-streaming-cursor"></span>
      </div>
    `;

    messagesContainer.appendChild(msgDiv);

    const contentElement = msgDiv.querySelector('.botsmith-streaming-content');
    const cursor = msgDiv.querySelector('.botsmith-streaming-cursor');

    // Preserve the API response as plain text while animating it safely

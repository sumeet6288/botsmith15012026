...
async def get_trend_analytics(
    ...
    """Get trend analytics for message volume over time with integration breakdown"""
    ...
    # Get message counts by date AND source (integration)
    message_pipeline = [
        {
            "$match": {
                "chatbot_id": chatbot_id,
    ...
    conversation_results = await db_instance.conversations.aggregate(conversation_pipeline).to_list(length=None)
    conversation_by_date = {item["_id"]: item["count"] for item in conversation_results}
    
    # Get message counts by date and source
    message_results = await db_instance.messages.aggregate(message_pipeline).to_list(length=None)
    
    # Organize messages by date and source
    messages_by_date_and_source = {}
    for item in message_results:
        date_str = item["_id"]["date"]
        source = item["_id"]["source"]
        count = item["count"]
        
        if date_str not in messages_by_date_and_source:
            messages_by_date_and_source[date_str] = {}
        messages_by_date_and_source[date_str][source] = count
    ...
    while current_date <= end_date:
        ...
        # Get integration-specific counts for this date
        source_counts = messages_by_date_and_source.get(date_str, {})
        
        # Calculate total messages for this date
        msg_count = sum(source_counts.values())
        
        data.append(TrendDataPoint(
            date=date_str,
            conversations=conv_count,
            messages=msg_count,
            dashboard=source_counts.get("dashboard", 0),
            widget=source_counts.get("widget", 0),
            telegram=source_counts.get("telegram", 0),
            slack=source_counts.get("slack", 0),
            discord=source_counts.get("discord", 0),
            whatsapp=source_counts.get("whatsapp", 0),
            instagram=source_counts.get("instagram", 0),
            messenger=source_counts.get("messenger", 0),
            msteams=source_counts.get("msteams", 0)
        ))

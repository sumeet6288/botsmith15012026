...
# Import BillingAuditService for PHASE 0 audit logging
from .billing_audit_service import billing_audit_service
...
class UsageService:
    ...
    async def increment_usage(
        ...
        if not result:
            ...
            # PHASE 0: Log limit exceeded event to audit log (fire-and-forget)
            try:
                asyncio.create_task(
                    billing_audit_service.log_limit_exceeded_event(
                        user_id=user_id,
        ...
        remaining = max(0, effective_limit - new_value)
        old_value = new_value - amount  # Calculate previous value
        ...
        # PHASE 0: Log to billing audit - Fire-and-forget (non-blocking)
        # This creates immutable audit trail for dispute resolution
        try:
            asyncio.create_task(
                billing_audit_service.log_usage_event(
                    user_id=user_id,

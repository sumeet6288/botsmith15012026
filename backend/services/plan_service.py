...
class PlanService:
    ...
    async def _invalidate_user_caches(self, user_id: str):
        ...
        cache_service.delete(f"usage_stats:{user_id}")
        
        # 2. Clear subscription cache
        cache_service.delete(f"subscription:{user_id}")
        cache_service.delete(f"subscription_status:{user_id}")
        
        # 3. Clear plan cache
        cache_service.delete(f"plan:{user_id}")
        
        # 4. Clear all user's chatbots cache
        chatbots_cursor = self.db.chatbots.find({"user_id": user_id})
        chatbots = await chatbots_cursor.to_list(length=100)
        for chatbot in chatbots:
            cache_service.delete(f"chatbot:{chatbot['id']}")
            cache_service.delete(f"public_chatbot:{chatbot['id']}")
    ...
    async def suspend_subscription(self, user_id: str, reason: str):
        """Suspend user subscription (immediate effect)"""
        await self.subscriptions_collection.update_one(
            {"user_id": user_id},
            {
                "$set": {
    ...
    async def change_user_plan(self, user_id: str, new_plan_id: str):
        """Admin changes user plan (immediate effect)"""
        await self.upgrade_plan(user_id, new_plan_id)
        
        # ✅ INVALIDATE CACHES (already called in upgrade_plan)

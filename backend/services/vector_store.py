...
class VectorStore:
    ...
    async def search(
        ...
        try:
            ...
            # Build query - only use $or if we have regex filters
            base_query = {"chatbot_id": chatbot_id}
            if regex_filters:
                base_query["$or"] = regex_filters
            
            # Get only chunks that match at least one query term (much faster than loading all)
            cursor = self.chunks_collection.find(base_query).limit(top_k * 3)  # Get 3x more for better scoring, but not all chunks
